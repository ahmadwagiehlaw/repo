...
function getCoverImage(caseItem) {
  if (caseItem.coverImage) return caseItem.coverImage;
  const attachments = Array.isArray(caseItem.attachments) ? caseItem.attachments : [];
  const imageAtt = attachments.find(a =>
    String(a.url || a.title || '').match(/\.(png|jpe?g|gif|webp)($|\?)/i) ||
    (a.attachmentType === 'file_cover')
  );
  return imageAtt?.url || null;
}
...
function getSessionTriple(caseItem) {
  ...
  const next     = future[0] || (caseItem?.nextSessionDate ? { date: caseItem.nextSessionDate, sessionType: caseItem.nextSessionType } : null);
...
export default function CasesList() {
  ...
  const [pageSize, setPageSize] = useState(() => Number(localStorage.getItem('lb_cases_pagesize')) || 12);
  ...
  const handleBulkRollover = async (action) => {
    ...
    let updates = getBulkRolloverUpdates(action);
    try {
      await Promise.all(Array.from(selectedCases).map((id) => storage.updateCase(workspaceId, id, updates)));
      setSelectedCases(new Set());
      window.location.reload();
    } catch (err) { console.error('Bulk Rollover failed', err); } 
    finally { setIsBulkRolling(false); }
  };
  ...
  const filteredAndSearched = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter((c) => {
        const searchable = [c.caseNumber, c.caseYear, c.plaintiffName, c.clientName, c.defendantName, c.court, getCaseSessionResult(c), c.lastSessionDate, c.nextSessionDate].join(' ').toLowerCase();
  ...
  const confirmFlagToggle = async () => {
    ...
    const currentFlags = mergedFlags(caseItem.flags, optimisticByCaseId[caseItem.id]);
    setOptimisticByCaseId((prev) => ({ ...prev, [caseItem.id]: { ...(prev[caseItem.id] || {}), [flagName]: nextValue } }));

    try {
      await updateFlags(targetWorkspaceId, caseItem.id, flagName, nextValue);
    } catch (error) {
      setOptimisticByCaseId((prev) => ({ ...prev, [caseItem.id]: { ...(prev[caseItem.id] || {}), [flagName]: Boolean(currentFlags[flagName]) } }));
    }
  };
  ...
  const handleDeleteCase = async (event, caseItem) => {
    ...
    const targetWorkspaceId = String(currentWorkspace?.id || caseItem?.workspaceId || '').trim();
    if (!targetWorkspaceId || !caseItem?.id) return;
    const confirmed = window.confirm('تأكيد حذف القضية نهائيًا؟ لا يمكن التراجع عن هذا الإجراء.');
    if (!confirmed) return;
    try {
      await storage.deleteCase(targetWorkspaceId, caseItem.id);
      await loadCases(targetWorkspaceId, 1000);
    } catch (error) { alert('تعذر حذف القضية حاليًا.'); }
  };
    ...
    <div>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <span style={{ color: 'var(--text-muted)', fontSize: '13px' }}>إجمالي: {filteredAndSearched.length} قضية</span>
          <button type="button" className="btn-secondary" style={{ fontSize: 13 }} onClick={() => setShowImporter(true)}>استيراد</button>
          <button type="button" className="btn-primary" onClick={() => setShowAddForm(true)}>إضافة قضية</button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', flex: 1 }}>
          ...
          {FILTER_CHIP_ITEMS.map((chip) => (
            <button key={chip.id} type="button" className={`filter-chip ${activeFilter === chip.id ? 'active' : ''}`} onClick={() => setFilter(chip.id)}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ fontSize: 13, lineHeight: 1 }}>{chip.icon}</span><span>{chip.label}</span></span>
            </button>
          ))}
          </div>

          {/* الشريط العائم الأنيق بجوار الفلاتر */}
          {selectedCases.size > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(8px)', border: '1px solid #e2e8f0', padding: '6px 12px', borderRadius: '12px', boxShadow: '0 4px 15px rgba(0, 0, 0, 0.05)', direction: 'rtl', flexWrap: 'wrap', animation: 'fadeIn 0.2s ease-out' }}>
              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#3b82f6', color: 'white', width: '22px', height: '22px', borderRadius: '50%', fontSize: '12px', fontWeight: 800 }}>
                {selectedCases.size}
              </span>
        ...
        <input className="form-input" placeholder="بحث سريع..." value={searchQuery} onChange={(event) => { setSearchQuery(event.target.value); setPage(1); }} style={{ width: 220, fontSize: 14 }} />
      </div>

      {!filteredAndSearched.length ? (
        <div className="empty-state"><div className="empty-state-icon">📁</div><div className="empty-state-text">لا توجد قضايا مطابقة</div></div>
      ) : (
        <>
          <div className="cases-grid">
            {paginatedCases.map((caseItem) => {
              ...
              const isVipDefendant = ['مدعى عليه', 'مطعون ضد', 'مستأنف ضد'].some((x) => String(roleCapacity).includes(x));
              const isNoInterest = String(roleCapacity).includes('لا شأن');
              const currentResult = triple.current ? (triple.current.result || triple.current.sessionResult || triple.current.courtDecision || '') : getCaseSessionResult(caseItem);
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    ...(isSelected ? { border: '2px solid #f59e0b', backgroundColor: '#fffbeb', transform: 'translateY(-2px)', boxShadow: '0 8px 20px rgba(245, 158, 11, 0.15)' }
                ...
                >
                  ...
                  <div className="case-card-row case-card-row--identity" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{
                          width: 36, height: 48, flexShrink: 0,
                          ...
                          {cover
                            ? <img src={cover} alt="غلاف" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  ...
                  <div className="case-card-row case-card-row--plaintiff">
                    <div className="case-plaintiff" style={{ filter: sensitiveHidden ? 'blur(4px)' : 'none', userSelect: sensitiveHidden ? 'none' : 'text', fontWeight: 900, fontSize: '15px' }}>{caseItem.plaintiffName || caseItem.clientName}</div>
                  </div>
                  {caseItem.defendantName && (
                    <div className="case-card-row case-card-row--defendant">
                      <span className="case-defendant" style={{ filter: sensitiveHidden ? 'blur(4px)' : 'none', userSelect: sensitiveHidden ? 'none' : 'text', fontWeight: 500, color: '#64748b' }}>ضد {caseItem.defendantName}</span>
                    </div>
                  )}

                  {/* ── ROW 4: Sessions Grid (الثلث والثلثين في سطرين فقط) ── */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginTop: '16px', padding: '10px 12px', background: isSelected ? 'rgba(255,255,255,0.6)' : '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                    {/* الثلث الأيمن: الجلسة القادمة */}
                    ...
                    {/* الثلثين الأيسر: الجلسة السابقة والقرار مدمجين */}
                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '13px', fontWeight: 800, color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>
                          {triple.current ? formatDisplayDate(triple.current.date) : (caseItem.lastSessionDate ? formatDisplayDate(caseItem.lastSessionDate) : '—')}
                        </span>
                        {currentResult && (
                          <span style={{ fontSize: '11px', color: '#1e3a8a', background: '#eff6ff', padding: '3px 8px', borderRadius: '4px', fontWeight: 700, flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={currentResult}>
                            {currentResult}
                          </span>
                        )}
                      </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, padding: '20px 0', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--text-secondary)', minWidth: 120, textAlign: 'center' }}>صفحة {page} من {totalPages} ({filteredAndSearched.length} قضية)</span>
            <button className="btn-secondary" disabled={page === totalPages} onClick={() => setPage((p) => p + 1)}>التالي</button>
            <select value={pageSize} onChange={(e) => { const val = Number(e.target.value); setPageSize(val); setPage(1); localStorage.setItem('lb_cases_pagesize', String(val)); }} style={{ padding: '6px 10px', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', fontFamily: 'Cairo', fontSize: 13, cursor: 'pointer' }}>
              <option value={10}>10 لكل صفحة</option><option value={20}>20 لكل صفحة</option><option value={50}>50 لكل صفحة</option>
            </select>
          </div>
        </>
      )}
      
      {/* Importer & Modals (same as before) */}
      {showImporter && (
      ...
      {showAddForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflowY: 'auto', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', width: '100%', maxWidth: '700px', marginTop: '20px' }}>
            ...
            <CaseForm caseData={null} workspaceId={workspaceId} onSave={async () => { setShowAddForm(false); await loadCases(workspaceId); }} onCancel={() => setShowAddForm(false)} />
          </div>
        </div>
      )}
      {confirmDialog && (
