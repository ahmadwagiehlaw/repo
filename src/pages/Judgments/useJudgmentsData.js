...
import { createAutoTaskPayload } from '@/engine/TaskEngine.js';
...
export function useJudgmentsData({ formatUiDate }) {
  const { loading: casesContextLoading, evaluateRules, getCase } = useCases();
  ...
  const handleCreateJudgment = async (caseItem) => {
    ...
    // WIRE-1: Evaluate rules after mutation
    const updatedCase2 = await getCase(workspaceId, caseId);
    if (updatedCase2 && evaluateRules) { await evaluateRules(workspaceId, updatedCase2); }

    if (isPlaintiff && appealDeadlineDate && !form.isFinal) {
      const autoAppealTask = createAutoTaskPayload({
        caseId,
      ...
      await storage.createTask(workspaceId, autoAppealTask);
    }
