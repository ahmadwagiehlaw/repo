...
export default function useCaseDetails() {
  ...
  const handleAddSession = useCallback(async () => {
    ...
    try {
      ...
      const newHistory = [...(caseData.sessionsHistory || []), newSession];
      const routingUpdate = getJudgmentReservationCaseUpdate(sessionFormData.sessionResult) || {};
      await storage.updateCase(workspaceId, caseId, {
        sessionsHistory: newHistory,
        ...routingUpdate,
      });
      setCaseData((prev) => (prev ? { ...prev, sessionsHistory: newHistory, ...routingUpdate } : null));
