...
export const storage = {
  ...
  /**
   * Get a Firebase Storage download URL for an attachment/file.
   * This returns a tokenized URL (not a custom backend signed URL).
   * @param {string} workspaceId
   * @param {string} caseId
   * @param {string} attachmentId
   * @returns {Promise<string>} download URL
   */
  async getAttachmentDownloadUrl(workspaceId, caseId, attachmentId) {
    if (!workspaceId || !caseId || !attachmentId) throw new Error('getAttachmentDownloadUrl: missing required parameters');
  ...
  async createCase(workspaceId, caseData) {
    ...
    // Enforce plan limits
    try {
      const { default: subscriptionManager } = await import('@/services/SubscriptionManager.js');
      const limit = subscriptionManager.getLimit('maxCases');
      if (limit !== -1) {
        ...
        if (snap.size >= limit) {
          throw new Error(
            `لقد وصلت للحد الأقصى (${limit} قضية) في خطتك المجانية. قم بالترقية إلى Pro للحصول على عدد غير محدود.`
          );
        }
      }
    } catch (limitError) {
      if (limitError.message.includes('الحد الأقصى')) throw limitError;
