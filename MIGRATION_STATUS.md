...
✅ Phase J Extension / Multi-Condition Action Engine
   - ترقية customReminderRules إلى condition1/condition2/actions مع دعم القواعد القديمة عند الحفظ
   - استبدال جدول القواعد في Settings.jsx بكروت شروط وأفعال
   - توسيع LB-DYNAMIC-CUSTOM-REMINDERS لدعم AND conditions وإنشاء المهام وتحديث الحقول وترحيل المسار
   - تسجيل hit لكل فعل آلي للشفافية
✅ Judgments Record UI
   - إعادة تصميم فلاتر سجل الأحكام ومبدّل الكروت/الجدول
   - تحويل كروت الأحكام إلى timeline متعدد الأحكام لكل قضية
   - تمييز الحكم التمهيدي/النهائي وأزرار إعادة المرافعة/الشعبة حسب آخر حكم
   - إعادة تنظيم JudgmentForm كشبكة عمودين مع إبراز اختيار تمهيدي/نهائي
   - ضبط Smart Action Bar لعرض إعادة المرافعة عند القضايا المحجوزة بلا حكم وتمرير isLatest لتفاصيل آخر حكم
   - إضافة fallback للأحكام المستوردة legacy وإعادة تصميم JudgmentDetails كملخص قانوني رسمي
   - تحويل كروت سجل الأحكام إلى responsive grid وإضافة semantic theme لعقد timeline
   - إضافة فلتر فرعي ديناميكي لتصنيفات الأحكام مع reset للصفحات ودعم legacy
   - تقوية فلتر التصنيف لقراءة legacy fields وإضافة زر تعديل فعلي داخل كروت الأحكام
   - إضافة طي/فتح للسجل التاريخي داخل timeline وتوحيد wrapper الفلاتر
   - إعادة تصميم رأس كارت الحكم لإظهار الخصوم وتخفيف شارات JudgmentDetails مع زر تعديل واضح
   - توحيد لوحة التحكم بسجل الأحكام وإضافة زر توسيع/طي جميع كروت الصفحة
   - تصحيح زر توسيع/طي الجميع ليحسب فقط القضايا ذات السجل التاريخي
   - إضافة وضع مصغّر افتراضي لكروت سجل الأحكام مع mini JudgmentDetails
✅ Templates / Case Documents
   - إضافة CRUD لمستندات القضايا داخل المسار workspaces/{workspaceId}/cases/{caseId}/documents
   - إضافة حفظ المستند من صفحة النماذج مباشرة داخل ملف القضية مع تحديث المستندات المحفوظة
   - إضافة تبويب "📄 المستندات" داخل تفاصيل القضية مع تعديل/طباعة/حذف
   - الإبقاء على محرر contentEditable الحالي للحفاظ على لصق Word، مع إضافة justify ومحاذاة الصور داخل الشريط
